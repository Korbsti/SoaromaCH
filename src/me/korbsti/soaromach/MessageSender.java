...
public class MessageSender {
	...
	public String format(String channelPrefix, Player p, String message,boolean isGlobal) {
		...
		if(isGlobal) {
			format = plugin.getConfig()
					.getString("channels.name.defaultGlobalMessageFormat");
			channelPrefix = "";
		}
	...
	public void messageChannelSender(Player player, String message, String permission, boolean isGlobal) {
			...
			if (p.hasPermission(permission)) {
				if (!plugin.getConfig().getBoolean(
					...
					if (plugin.getConfig().getBoolean("channels.name." + plugin.currentChannel.get(player.getName())
						...
						p.sendMessage(
								ChatColor.translateAlternateColorCodes('&', format(prefixChannel, player, message, isGlobal)));
					} else {
						if (plugin.currentChannel.get(p.getName()) == plugin.currentChannel.get(player.getName())) {
							p.sendMessage(ChatColor.translateAlternateColorCodes('&',format(prefixChannel, player, message, isGlobal)));
						}
					}
						...
						public void run() {
								...
								if (entity instanceof Player) {
									...
									if (plugin.getConfig()
										...
										playerd.sendMessage(ChatColor.translateAlternateColorCodes('&',
												format(prefix, player, message, isGlobal)));
									} else {
											...
											playerd.sendMessage(
													ChatColor
															.translateAlternateColorCodes(
																	'&', format(
																			plugin.getConfig()
																					.getString("channels.name." + plugin.currentChannel.get(player.getName())+ ".prefix"), player, message, isGlobal)));
										}
									}
							...
							if (plugin.getConfig()
									.getBoolean("channels.name." + plugin.currentChannel.get(player.getName())
											+ ".sendRegardlessOfCurrentChannel") == true) {
								player.sendMessage(
										ChatColor.translateAlternateColorCodes('&', format(prefix, player, message, isGlobal)));
							} else {
		...
		Bukkit.getLogger().info(ChatColor.translateAlternateColorCodes('&', format(prefixChannel, player, message, isGlobal)));
	}
}
